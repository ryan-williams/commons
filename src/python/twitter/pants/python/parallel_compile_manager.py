...
class ParallelCompileManager(object):
  ...
  def _handle_processed_node_set(self, target_node_set, was_compiled, was_successful):
    ...
    print "In flight (%d): {%s}. " % (len(self._in_flight_target_nodes), ','.join([t.short_id for t in self._in_flight_target_nodes])),
    print "Frontier (%d): {%s}" % (len(self._frontier_nodes), ','.join(t.short_id for t in self._frontier_nodes))
